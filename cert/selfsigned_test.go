...
func TestTLSDisableAutofill(t *testing.T) {
	...
	require.NoError(
		t, err,
		"unable to read tls certificate",
	)

	keyBytes, err := ioutil.ReadFile(keyPath)
	require.NoError(
		t, err,
		"unable to read tls key",
	)
...
func TestTlsConfig(t *testing.T) {
	...
	_, _, err = cert.GenCertPair(
		...
		[]string{extraDomains[0]}, cert.DefaultAutogenValidity, false, false,
		keyRing, "ec",
	)
	if err != nil {
		t.Fatal(err)
...
func TestEncryptedTlsConfig(t *testing.T) {
	...
	_, _, err = cert.GenCertPair(
		"lnd autogenerated cert", certPath, keyPath, []string{extraIPs[0]},
		[]string{extraDomains[0]}, cert.DefaultAutogenValidity, true, true,
		keyRing, "ec",
	)
	if err != nil {
		t.Fatal(err)
